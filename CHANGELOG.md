...
- **Auto-response macros ([#364](https://github.com/gbmhunter/NinjaTerm/issues/364)).** Each macro has new optional *Send on connect* and *Send on RX match (regex)* triggers in its settings modal. RX matches test each finalised line from the wire (TX echo is ignored, so a macro can't loop on its own response). V1 is ASCII-regex only; hex byte matching is a follow-up. Wiring: `MacroController.onRxBytes` fed from `App.parseRxData`, a `connState` reaction in `App` for the connect/disconnect hooks, four new fields on `Macro` + `MacroDataV1`, seeded with defaults by the unreleased v18 migration.
