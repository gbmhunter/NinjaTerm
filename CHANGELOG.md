...
- Suppressed the SEGGER J-Link "Probe selection" / "Target device settings" Windows MessageBox dialogs that the DLL would pop on connection failures (e.g. clicking Open with no probe attached, or unplugging the dev kit mid-session). The DLL has its own default error/warning handlers that fall back to MessageBox; NinjaTerm now installs no-op handlers via `JLINKARM_SetErrorOutHandler` / `JLINKARM_SetWarnOutHandler` at library load time and routes the messages through to the in-app diagnostic pane instead.
