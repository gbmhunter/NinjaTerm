...
- **Closing the window now stops the CPU-usage monitor and the flow-control poll.** `stopCpuMonitoring` was an empty method and `ConnController.cleanup` never cleared the 1s `getFlowControlSignals` interval; both are now cancelled, and the poll logs rather than throws on a bad IPC response.
