...
- `1` — bold / increased intensity (also brightens the foreground color)
- `30`–`37` — foreground color, `90`–`97` — bright foreground color
- `40`–`47` — background color, `100`–`107` — bright background color

| Color | Foreground | Bright foreground | Background | Bright background |
| --- | --- | --- | --- | --- |
| Black | 30 | 90 | 40 | 100 |
| Red | 31 | 91 | 41 | 101 |
| Green | 32 | 92 | 42 | 102 |
...
For example `ESC[31;44m` is red on blue, and `ESC[91;44;1m` is bright red on blue, bold. These can override the default background/TX/RX colors set in `Settings > Display`.
...
Which to use is really a question about your firmware's source file, not your device. If your toolchain saves source as UTF-8 (STM32CubeIDE does, for example) you have three options:

- **Recommended:** pick **CP437** in NinjaTerm, write the box-drawing characters directly in your source (`printf("┌───┐")`), and tell the compiler to emit them as CP437. With GCC, add `-fexec-charset=cp437` to the compiler flags (in STM32CubeIDE: Project Properties → C/C++ Build → Settings → MCU GCC Compiler → Miscellaneous). The source stays readable and each box-drawing character is one byte in flash and on the wire.
- Pick **UTF-8** in NinjaTerm and write the characters directly in your source, with no compiler flag. The compiler encodes them as UTF-8 and NinjaTerm decodes them back — but each box-drawing character costs three bytes.
- Pick **CP437** and emit the raw bytes with hex escapes: `printf("\xDA\xC4\xC4\xC4\xBF")`. One byte per character with no compiler flag, but harder to read in the source. The byte values are in the reference below.
...
### CP437 Box-Drawing Reference

The 40 box-drawing characters in CP437, as the byte your device sends. Decimal is what a Windows Alt code uses; the hex form is what you write in a C string.

**Single line**

| Glyph | Decimal | Hex | Glyph | Decimal | Hex |
| :---: | --- | --- | :---: | --- | --- |
| ┌ | 218 | `\xDA` | ┐ | 191 | `\xBF` |
| └ | 192 | `\xC0` | ┘ | 217 | `\xD9` |
| ─ | 196 | `\xC4` | │ | 179 | `\xB3` |
...
:::note
Typing these on Windows with Alt + decimal on the numpad goes through Windows' *OEM* code page, which is CP437 only on a US install. Many European installs use CP850, where the 18 mixed single/double characters (181–184, 189, 190, 198, 199 and 207–216) are replaced by accented letters, so those Alt codes type the wrong character. Use the hex escapes instead, or copy the glyphs from the tables above.
:::
