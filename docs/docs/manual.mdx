...
| Option | Description |
| --- | --- |
| ASCII only (default) | 7-bit ASCII. Bytes `0x80`+ aren't text, so they're shown as hex glyphs (or swallowed) per the non-visible character setting. Best for simple use cases and general purpose debugging. |
...
Which to use depends on your firmware source code and build settings. If your toolchain saves source as UTF-8 (STM32CubeIDE does, for example) you have three options:
...
- Pick **UTF-8** in NinjaTerm and write the characters directly in your source, with no compiler flag. The compiler encodes them as UTF-8 and NinjaTerm decodes them back. Just be aware that each box-drawing character costs three bytes.
