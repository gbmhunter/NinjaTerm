      ...
      >
        ...
        <Grid xs={12} sx={{ height: '20px' }} />
          ...
          <ol>
            ...
            <li><b>An advanced ASCII text command-based protocol</b> for complex multi-plot scenarios: Special commands like <code>$NT:GPH:ADD_FIG ...</code> and <code>$NT:GPH:ADD_DATA ...</code> can be sent from the other end of the serial connection to create and update plots in NinjaTerm.</li>
          </ol>
...
2025-08-12 12:00:07 - Temperature: 26.1 degC`}
          ...
          <p>All graphing related commands start with <code>$NT:GPH:</code>. The command does not need be at the start of a graphing frame (i.e. random data can occur before the <code>$NT:GPH:</code> command). In Advanced Cmd Mode, NinjaTerm starts buffering when <code>$NT</code> is seen and processes the command when a <code>;</code> is received that is not inside double quotes.</p>
...
{`$NT:GPH:ADD_TRACE,fig=fig1,id=temp,name="Temperature",color=#FF0000,xtype=timestamp;`}
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>The type of data to use for the x-axis. There are three options:
                  <ul>
                    <li><code>timestamp</code>: The x-axis will be the time data arrived at NinjaTerm. In the <code>GPH:ADD_DATA</code> command you supply y-values only. Works well when you a slowly sending single values back per <code>GPH:ADD_DATA</code> command (e.g. reading a temperature sensor once per second).</li>
                    <li><code>counter</code>: The x-axis will be a counter that automatically increments (0, 1, 2, ...) for each received data point for that trace. In the <code>GPH:ADD_DATA</code> command you supply the y-values only. Works well for arrays of data where each point has been sampled at a regular interval (e.g. an ADC taking 1024 samples and returning all the data in a single <code>GPH:ADD_DATA</code> command).</li>
                    <li><code>data</code>: The x-axis will be the data itself. In this case you have to provide both the x and y values in the <code>GPH:ADD_DATA</code> command. Works well for scatter plot style data.</li>
                  </ul>
                </td>
              </tr>
            </tbody>
          ...
          <p>Once you have created a plot and a trace on the plot, use the <code>GPH:ADD_DATA</code> command to add data points to the trace. This will draw the points on the graph.</p>
          ...
          <table className="documentation-table">
            ...
            <tbody>
              <tr>
                ...
                <td>Required</td>
                <td>The trace to add the data to. The trace must have been created prior with the <code>GPH:ADD_TRACE</code> command.</td>
              </tr>
              <tr>
          ...
          <p>Use when you want x values to be the time data arrives at NinjaTerm. This works best when you are sending single values over per <code>GPH:ADD_DATA</code> command, such as temperature sensor samples once per second.</p>
          <pre style={{'backgroundColor': '#333', 'padding': '15px', 'borderRadius': '5px', 'overflowX': 'auto'}}>
{`$NT:GPH:ADD_DATA,trace=temp,data=[1.23];`}
          </pre>

          <p>You are allowed to send an extra comma after the last data value (IMO all data formats should allow this, I'm looking at you, JSON and C++ initializer lists!), so you don't have to add conditional logic in your firmware to not generate the comma on the last data value.</p>
...
$NT:GPH:ADD_DATA,trace=pressure,data=[1013.1,1012.9,1013.3];`}
          ...
          <ul>
            <li>Commands must start with <code>$NT:GPH:</code> and end with an unquoted <code>;</code></li>
            ...
            <li>In Basic Prefix Mode, <code>$NT:GPH:</code> commands are processed when the configured processing trigger (e.g., LF) is received</li>
            ...
            <li>Multiple data points in a single ADD_DATA command can be separated by pipes (for x,y pairs) or commas (for y-only values)</li>
          </ul>
